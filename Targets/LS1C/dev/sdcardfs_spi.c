...
{
if(bit) 
	SET_SPI(0x5, 0xF5);		//cs high
else 
	SET_SPI(0x5, 0xB5);		//cs low
}
...
unsigned int SD_Get_CardInfo(unsigned char *ReadBuffer)//Read CSD register
{
 ...
 //     while(flash_writeb_cmd(0xff) != 0xfe)//fsy
//	       ;
	flash_writeb_cmd(0xff);
	flash_writeb_cmd(0xff);
	flash_writeb_cmd(0xff);
